 ...
 */
public class MyRuleGenWithTimeConstraints {
	 ...
	 * Default constructor.
	 */
	public MyRuleGenWithTimeConstraints() {
	}
	 ...
	 */
	public List<Pair<Sequence, Sequence>> runAlgorithm(double minsup, double minconf,
		...
		AlgoFournierViger08 algo = new AlgoFournierViger08(
				...
				algoClustering,
				findClosedPatterns,
				enableBackscanPruning);
		Sequences patternsLists = algo.runAlgorithm(sequenceDatabase);

		// STEP 2: Generate  rules of the form    a ==> b, 
		// where a and b are sequential patterns 
		// such that a is a subsequence of b.
		// For each rule
		List<Pair<Sequence, Sequence>> result = new ArrayList<>();
			...
			for(int j=0; j < patternsLists.getLevel(i).size(); j++){
				...
				for (int k = i+1; k < patternsLists.getLevels().size(); k++) {
					for(int m =0; m < patternsLists.getLevel(k).size(); m++){
						...
						tryToGenerateRule(pattern1, pattern2, minconf, sequenceDatabase.size(), decodeFunction).ifPresent(result::add);
						// try to generate a rule   b ==> a
						tryToGenerateRule(pattern2, pattern1, minconf, sequenceDatabase.size(), decodeFunction).ifPresent(result::add);
					}
				}
			}
		}
	 ...
	 * @throws IOException 
	 */
	private Optional<Pair<Sequence, Sequence>> tryToGenerateRule(Sequence pattern1,
																				   Sequence pattern2,
																				   double minconf,
																				   int sequenceCount,
																				   Function<Integer, String> decodeFunction) throws IOException {
		...
		Map<Integer, Long> pattern1map = pattern1.getItemsets().stream()
				.flatMap(x -> x.getItems().stream().map(ItemSimple::getId))
...
//
	...
	public String itemsetsToString(Sequence pattern, Function<Integer, String> decodeFunction) {
		StringBuilder r = new StringBuilder("");
		for(Itemset itemset : pattern.getItemsets()){
			r.append('{');
			for(ItemSimple item : itemset.getItems()){
				String string = decodeFunction.apply(item.getId());
				r.append(string);
				r.append(' ');
			}
			r.append('}');
	 ...
	 */
	boolean strictlyContains(Sequence pattern1, Sequence pattern2) {
