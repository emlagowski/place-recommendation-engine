...
writeTransactions(mergedNycWithCategories, c('UserId', 'timestamp', 'VenueCategory'))
sequences <- readTransactions()
```

```{r}
rules <- mineRules(sequences, support = 0.1, confidence = 0.1, mingap = second, maxgap = 14 * day)
rules.noSubsetOnRight <- removeRightEqualToLeft(rules)

#rules.no_redundant <- rules.noSubsetOnRight[is.redundant(rules.noSubsetOnRight)]

rules.filtered <- subset(rules.noSubsetOnRight, lift > 0.1)

rules.sorted <- sort(rules.filtered, by=c('lift', 'support', 'confidence'))

inspect(head(rules.sorted, 1000))
