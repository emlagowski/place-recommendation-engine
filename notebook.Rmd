...
#nyc$timestamp <- as.POSIXlt(nyc$utcTimestamp, format = "%a %b %d %H:%M:%S +0000 %Y", tz="UTC")
#nyc <- nyc[order(nyc$userId,nyc$timestamp),]
#write.table(nyc, "./data/dataset_tsmc2014/nyc.data", sep = "," , row.names = FALSE, col.names = FALSE )

#str(nyc)

#nyc_trs <- nyc[,c(1,10,3)]
#nyc_trs <- nyc_trs[order(nyc_trs$userId, nyc_trs$timestamp),]
#sapply(nyc_trs, class)
#write.table(nyc_trs, "./data/dataset_tsmc2014/nyc_trs.data", sep = "," , row.names = FALSE, col.names = FALSE )

#str(nyc_trs)

#(max(nyc_trs$timestamp)-min(nyc_trs$timestamp))

#checkinsByUser <- aggregate(UTC ~ UserID, nyc, length)
#colnames(checkinsByUser) <- c("UserID", "Checkins")
#checkinsByUser[order(-checkinsByUser$Checkins),]
```
...
write.table(nyc_wo_duplicates, "./data/dataset_tsmc2014/nyc_wo_duplicates.data", sep = "," , row.names = FALSE, col.names = FALSE )

dataSeq <- read_baskets(con = "./data/dataset_tsmc2014/nyc_wo_duplicates.data", sep = ",", info = c("sequenceID", "eventID"))
...
seqParam = new ("SPparameter",support = 0.2, maxsize = 6, mingap=1, maxgap = 170000, maxlen = 6)
