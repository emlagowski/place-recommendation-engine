...
venueFreq <- data.frame(table(nyc$venueId))
nyc$venuePopularity = venueFreq$Freq[match(nyc$venueId, venueFreq$Var1)]
venueCategoryFreq <- data.frame(table(nyc$venueCategoryId))
nyc$venueCategoryPopularity = venueCategoryFreq$Freq[match(nyc$venueCategoryId, venueCategoryFreq$Var1)]
nyc$venyePopularityToCategory = nyc$venuePopularity / nyc$venueCategoryPopularity 
allEvents <- nrow(nyc)
nyc$venyePopularityToAll = nyc$venuePopularity / allEvents

# 1. wybierz metodę
#discretizeMethod <- "frequency"
discretizeMethod <- "interval"

# 2. wybierzilośc stopni
categories <- c("super_low", "low", "medium", "high", "super_high")
#categories <- c("low", "medium", "high")

categoriesLen <- length(categories)

nyc$venuePopularityToCategoryDiscrete <- discretize(nyc$venyePopularityToCategory, categories = categoriesLen, method = discretizeMethod, labels = categories)
summary(nyc$venuePopularityToCategoryDiscrete)
nyc$venuePopularityToAllDiscrete <- discretize(nyc$venyePopularityToAll, categories = categoriesLen, method = discretizeMethod, labels = categories)
summary(nyc$venyePopularityToAllDiscrete)

# 3. wybierz stopniowanie toAll / toCategory
nyc$MERGED = paste(nyc$venueCategory, nyc$venuePopularityToCategoryDiscrete, sep="=")
#nyc$MERGED = paste(nyc$venueCategory, nyc$venuePopularityToAllDiscrete, sep="=")
...
# 4. wybierz czy używana będzie popularność
# with MERGED
nyc_wo_duplicates <- nyc_wo_duplicates[,c(1,10,17)]
# with VenueCategory
...
print('venyePopularityToCategory')
summary(nyc$venyePopularityToCategory)
plot(nyc$venyePopularityToCategory)
#hist(nyc$venyePopularityToCategory)
print('venyePopularityToAll')
summary(nyc$venyePopularityToAll)
plot(nyc$venyePopularityToAll)
#hist(nyc$venyePopularityToAll)
print('venuePopularityToCategoryDiscrete')
summary(nyc$venuePopularityToCategoryDiscrete)
plot(nyc$venuePopularityToCategoryDiscrete)
#hist(nyc$venuePopularityToCategoryDiscrete)
print('venuePopularityToAllDiscrete')
summary(nyc$venuePopularityToAllDiscrete)
plot(nyc$venuePopularityToAllDiscrete)
#hist(nyc$venuePopularityToAllDiscrete)
```
...
hour = 60 * minute
day = 24 * hour
seqParam = new ("SPparameter", support = 0.1, maxsize = 6, mingap = 1 * minute, maxgap = 2 * day, maxlen = 6)
